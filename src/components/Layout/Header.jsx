...
const Header = () => {
  ...
  return (
    <header className={`fixed w-full z-50 transition-all duration-300 bg-white shadow-xl ${isScrolled ? 'py-3' : 'py-5'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          <div className="relative">
            ...
            <div className="absolute -inset-1 bg-primary/10 blur-lg rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          </div>
        ...
        <nav className="hidden lg:flex items-center gap-10">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className={`text-xs font-bold uppercase tracking-widest transition-all hover:text-primary relative group ${isActive(link.path) ? 'text-primary' : 'text-secondary'}`}
            >
              {link.name}
        ...
        {/* Mobile Menu Button */}
        <button className="lg:hidden p-2 hover:bg-black/5 rounded-full transition-colors text-secondary" onClick={() => setIsOpen(!isOpen)}>
